...
function _parseKey1(refstr, info)
{
	...
	var joinLen = +info[1];
	if (isNaN(joinLen)) throw new Error('JoinIndexs Length Is NaN');
	var subtypeLen = +info[joinLen+2];
	if (isNaN(subtypeLen)) throw new Error('Subtype Length Is NaN');
	...
	var joinIndexs = headArr.slice(2, joinLen+2).map(function(val)
		{
			if (isNaN(val)) throw new Error('JoinIndex Is NaN');
			return +val;
		});
