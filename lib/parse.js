...
function _oneRef_msgid_msgstr(ref, msgid, msgstr)
{
	...
	if (ref && ref.substr(0, 2) != '@@') return;

	var info = ref ? refsUtils.parse(ref.substr(2)) : {};
	...
	if (info.subtype)
	{
		var translateData = refsUtils.mixMsgsByJoinIndexs(msgid, msgstr, info.joinIndexs);
		extendData1[info.subtype] = translateData;
