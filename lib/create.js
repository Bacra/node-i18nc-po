...
// 为了兼容原来的返回，create继续返回string
exports.create = create;
function create()
{
	var ret = createPO.apply(this, arguments);
	var result = {po: {}, pot: ret.pot.toString()};

	_.each(ret.po, function(val, key)
	{
		result.po[key] = val.toString();
	});
...
function createPO(json, options)
{
	...
	{
		pickFileLanguages = json.allFuncLans();
		ArrayPush.apply(pickFileLanguages, json.allUsedLans());
	}

	debug('lans:%o', pickFileLanguages);
...
function filterTranslateWords(json, lan, existedTranslateFilter)
{
	...
	{
		var subtypeAstMap = {};
		json.words.codeTranslateWords.list.forEach(function(item)
		{
