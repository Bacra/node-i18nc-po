...
function filterTranslateWords(json, lan, existedTranslateFilter)
{
	...
	var usedTranslateWords = words.usedTranslateWords && words.usedTranslateWords[lan] || {};
	...
	if (hasLineSubtype)
	{
		...
		_.each(lineSubtypeList, function(subtypeList, mainSubtype)
		{
			...
			mySubtypeAstMap.sort(function(a, b)
				{
					return a.orignalAst.range[0] > b.orignalAst.range[0] ? 1 : -1;
				})
...
function _getCodeUsedLans(json)
{
	...
	_.each(words.funcTranslateWords, function(lansList)
	{
		_.each(lansList, function(lans)
		{
			result.push.apply(result, _.keys(lans));
		});
	});

	_.each(words.usedTranslateWords, function(words, lan)
	{
		result.push(lan);
	});
