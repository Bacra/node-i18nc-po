...
exports.create = function create(json, options)
{
	...
	if (options.pickFileLanguages && options.pickFileLanguages.length)
	{
		...
		{
			var items = getTranslateWords(json, lan);
...
function getTranslateWords(json, lan)
{
	var fileKey = json.currentFileKey;
	var items = {};
	var translateData = json.usedTranslateWords[lan] || {};
	_.each(json.codeTranslateWords.DEFAULTS, function(msgid)
	{
...
_.extend(TPOItem.prototype,
{
	toPOItem: function()
	{
		...
		item.comments = _.map(this.fileInfos, function(num, fileInfo)
			{
				return '@@:'+fileInfo;
			});

		debug('item comments:%o', item.comments);
	...
	addKeyInfo: function(fileKey, subtype, joinstr)
	{
		...
		if (subtype)
		{
			...
			debug('subtype2fileinfo %s', fileInfo);
		}
		else
		{
			fileInfo = '0,'+fileKey;
